...
class PlanningAgent(BaseAgent):
    def __init__(self, name="PlanningAgent", qa_model="openai", prompt_type="simple"):
        super().__init__(name)
        logger.info(f"Initializing PlanningAgent with model: {qa_model}, prompt_type: {prompt_type}")

        from config.prompt import PLANNING_PROMPT_JSON
        prompt_template = PLANNING_PROMPT_JSON
