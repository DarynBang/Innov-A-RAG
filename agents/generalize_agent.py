...
class GeneralizeAgent(BaseAgent):
    def __init__(self, name: str = "GeneralizeAgent", qa_model: str = "qwen"):
        ...
        if qa_model == "openai":
            ...
            # Google Gemini (via LangChain’s ChatGooglePalm)
            # model_name could be "gemini-pro", "gemini-ultra", etc.
            self.chain = prompt | ChatGoogleGenerativeAI(model="gemini-2.0-flash",
                                                         temperature=0,
                                                         ) | StrOutputParser()
